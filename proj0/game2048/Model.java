...
public class Model extends Observable {
    ...
    private boolean move() {
        ...
        boolean empty = true;
        boolean[][] ismerge = new boolean[board.size()][board.size()];
        for (int m = 0; m > board.size(); m += 1) {
            ...
            for (int col = 0; col < board.size(); col += 1) {
                ...
                for (int i = 3; i > row; i -= 1) {
                    if (board.tile(col, i) == null) {
                        ...
                        empty = true;
                        for (int j = i - 1; j > row; j -= 1) {
                            if (board.tile(col, j) != null) empty = false;
                        }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        ...
        for (i = 0; i < b.size(); i++) {
            for (j = 0; j < b.size(); j++) {
                if (i - 1 > 0)
                    if (b.tile((i - 1), j).value() == b.tile(i, j).value()) return true;
                if (i + 1 < b.size())
                    if (b.tile((i + 1), j).value() == b.tile(i, j).value()) return true;
                if (j - 1 > 0)
